...
const festivalStartDate = new Date("2025-02-17T01:40:40").getTime();
const festivalEndDate = new Date("2025-04-27T15:00:00").getTime();
...
const getTimeLeft = (): CountdownProps => {
    ...
    if (now >= festivalStartDate) return {
    ...
    return {
        statusMessage: `Shred Fest 5 is coming up in ${dayjs().to(festivalStartDate, true)}! 🎉`,
        timeLeft: { d, h, m, s },
                    ...
                    {(isVisible && (countdown.status == "timeRemaining")) && (
                        <div className="ion-padding crazy-bg" onClick={onToggleView}>
                            <div className="countdown-container">
                                {countdown.timeLeft && Object.entries(countdown.timeLeft).map(([unit, value]) => (
                                    <span key={unit} className="countdown-item">
                                        ...
                                        <span className="countdown-label crazy-text">{unit}&nbsp;</span>
                                    </span>
                                ))}
                            </div>
                        </div>)}
